...
void ZipDirectory(const std::string& source_path,
  ...
  if (zipper == nullptr) {
    ...
    throw std::runtime_error("Failed to open destination archive " +
                             destination_path + ": " +
...
void UnzipDirectory(const std::string& archive_path,
  ...
  std::unique_ptr<zip_t, decltype(zip_deleter)> zipper(
      zip_open(archive_path.c_str(), ZIP_RDONLY, &errorp), zip_deleter);
  if (zipper == nullptr) {
    zip_error_t ziperror;
    zip_error_init_with_code(&ziperror, errorp);
  ...
  for (int i = 0; i < zip_get_num_entries(zipper.get(), 0); ++i) {
    ...
    if (zip_stat_index(zipper.get(), i, kNoZipFlags, &stat) < 0) {
      throw std::runtime_error("Failed to stat archive entry: " +
                               std::string(zip_strerror(zipper.get())));
    }
    ...
    if (path.back() != '/') {
      std::unique_ptr<zip_file_t, decltype(zip_file_deleter)> ifile(
          zip_fopen_index(zipper.get(), i, kNoZipFlags), zip_file_deleter);
