...
TEST_F(TrafficPoseSelectorDragwayTest, TwoLaneDragwayAutoDiff) {
  ...
  const AutoDiffXd scan_ahead_distance(kDragwayLaneLength / 2.);
  {
    const std::map<AheadOrBehind, const ClosestPose<AutoDiffXd>> closest_poses =
        TrafficPoseSelector<AutoDiffXd>::FindClosestPair(get_lane(ego_pose, *road_), ego_pose, traffic_poses,
                                                         scan_ahead_distance, ScanStrategy::kPath);

    // Verifies that the correct leading and trailing cars are identified, and
    // that their derivatives are correct.
    const Vector3<AutoDiffXd> srh_ahead = closest_poses.at(AheadOrBehind::kAhead).odometry.pos.srh();
    ...
    for (int i{0}; i < 3; ++i) {
      EXPECT_TRUE(test::CompareMatrices(expected_derivatives.col(i), srh_ahead(i).derivatives()));
    }
    const Vector3<AutoDiffXd> srh_behind = closest_poses.at(AheadOrBehind::kBehind).odometry.pos.srh();
    EXPECT_EQ(kJustBehindSPosition, srh_behind.x().value());
    for (int i{0}; i < 3; ++i) {
      EXPECT_TRUE(test::CompareMatrices(expected_derivatives.col(i), srh_behind(i).derivatives()));
    }
    const AutoDiffXd dist_ahead = closest_poses.at(AheadOrBehind::kAhead).distance;
    EXPECT_EQ(kJustAheadSPosition - kEgoSPosition, dist_ahead);
    // Distance is always invariant wrt. to x, y and z.
    EXPECT_TRUE(test::CompareMatrices(Vector3<double>(0., 0., 0.), dist_ahead.derivatives()));
    const AutoDiffXd dist_behind = closest_poses.at(AheadOrBehind::kBehind).distance;
    EXPECT_EQ(kEgoSPosition - kJustBehindSPosition, dist_behind);
    // Distance is always invariant wrt. to x, y and z.
    EXPECT_TRUE(test::CompareMatrices(Vector3<double>(0., 0., 0.), dist_behind.derivatives()));
  }

  // Bump the "just ahead" car into the lane to the left.
  Isometry3<AutoDiffXd> isometry_just_ahead = traffic_poses.get_pose(kJustAheadIndex);
  isometry_just_ahead.translation().y() += kDragwayLaneWidth;
  traffic_poses.set_pose(kJustAheadIndex, isometry_just_ahead);
  // Peer into the adjacent lane to the left.
  {
    ...
    const Vector3<AutoDiffXd> srh_behind = closest_poses.at(AheadOrBehind::kBehind).odometry.pos.srh();
    EXPECT_EQ(-std::numeric_limits<AutoDiffXd>::infinity(), srh_behind.x().value());
    for (int i{0}; i < 3; ++i) {
      EXPECT_TRUE(test::CompareMatrices(Vector3<double>(0., 0., 0.), srh_behind(i).derivatives()));
    }
    const AutoDiffXd dist_ahead = closest_poses.at(AheadOrBehind::kAhead).distance;
    EXPECT_EQ(kJustAheadSPosition - kEgoSPosition, dist_ahead);
    // Distance is always invariant wrt. to x, y and z.
