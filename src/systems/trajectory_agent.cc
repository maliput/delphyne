...
#include "systems/trajectory_agent.h"
...
template <typename T>
TrajectoryAgent2<T>::TrajectoryAgent2(const AgentData2& agent_data,
                                    const AgentTrajectory& trajectory,
                                    double sampling_time_sec)
    : systems::LeafSystem<T>(
          systems::SystemTypeTag<automotive::TrajectoryAgent2>{}),
      agent_data_(agent_data),
      trajectory_(trajectory) {
  this->DeclarePeriodicUnrestrictedUpdate(sampling_time_sec, 0.);
  this->DeclareVectorOutputPort(&TrajectoryAgent2::CalcStateOutput);
  this->DeclareVectorOutputPort(&TrajectoryAgent2::CalcPoseOutput);
  this->DeclareVectorOutputPort(&TrajectoryAgent2::CalcVelocityOutput);
}

template <typename T>
void TrajectoryAgent2<T>::CalcStateOutput(
...
template <typename T>
void TrajectoryAgent2<T>::CalcPoseOutput(const systems::Context<T>& context,
...
template <typename T>
void TrajectoryAgent2<T>::CalcVelocityOutput(const systems::Context<T>& context,
...
template <typename T>
PoseVelocity TrajectoryAgent2<T>::GetValues(
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS(
    class ::drake::automotive::TrajectoryAgent2)
