...
// making will be constrained to whatever this class is capable of
  ...
  Eigen::Quaternion<double> zero_heading(Eigen::AngleAxis<double>(0.0, Eigen::Vector3d::UnitZ()));
  std::vector<Eigen::Quaternion<double>> orientations(5, zero_heading);
  double y = -5.55;
  std::vector<Eigen::Vector3d> translations {Eigen::Vector3d(0.0, y, 0.0), Eigen::Vector3d(10.0, y, 0.0),
                                             Eigen::Vector3d(30.0, y, 0.0), Eigen::Vector3d(60.0, y, 0.0),
                                             Eigen::Vector3d(100.0, y, 0.0)};
  drake::automotive::AgentTrajectory trajectory = drake::automotive::AgentTrajectory::Make(times, orientations,
                                                                                           translations);
  ...
  std::unique_ptr<drake::automotive::TrajectoryFollower<double>> system = std::make_unique<
      drake::automotive::TrajectoryFollower<double>>(trajectory, sampling_time);
  system->set_name(name_);
  trajectory_follower_system_ = builder->template AddSystem<drake::automotive::TrajectoryFollower<double>>(
      std::move(system));
  ...
  car_vis_applicator->AddCarVis(std::make_unique<drake::automotive::PriusVis<double>>(id, name_));

  /*********************
   * State Publisher
   *********************/
  auto agent_state_translator = builder->template AddSystem<DrakeSimpleCarStateToIgn>();

  const std::string agent_state_channel = "agents/" + std::to_string(id) + "/state";
  auto agent_state_publisher = builder->AddSystem<IgnPublisherSystem<ignition::msgs::SimpleCarState>>(
      agent_state_channel);

  // Drake car states are translated to ignition.
  builder->Connect(trajectory_follower_system_->state_output(), agent_state_translator->get_input_port(0));
