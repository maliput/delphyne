...
GTEST_TEST(TrajectoryFollowerTest, Outputs) {
  ...
  for (const auto& it : test_cases) {
    ...
    std::unique_ptr<drake::systems::SystemOutput<double>> outputs =
...
GTEST_TEST(TrajectoryFollowerTest, ToAutoDiff) {
  ...
  EXPECT_TRUE(
      is_autodiffxd_convertible(follower, [&](const auto& autodiff_dut) {
