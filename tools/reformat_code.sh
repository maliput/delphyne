...
find . -not \( -path ./bridge/drake -prune \) -not \( -path ./bridge/protobuf -prune \) -not \( -path ./test/gtest -prune \) -iname '*.cc' -o -iname '*.cpp' -o -iname '*.c' -o -iname '*.hpp' -o -iname '*.hh' -o -iname '*.h' | while read file ; do
