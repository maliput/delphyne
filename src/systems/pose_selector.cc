...
template <typename T>
LaneDirection CalcLaneDirection(const Lane* lane,
...
template <typename T>
RoadOdometry<T> MakeInfiniteOdometry(const LaneDirection& lane_direction,
...
ClosestPose<T> FindSingleClosestInDefaultPath(
  ...
  LaneDirection lane_direction = CalcLaneDirection<T>(
...
ClosestPose<T> FindSingleClosestInBranches(
    ...
    const PoseBundle<T>& traffic_poses, const T& scan_distance,
    const AheadOrBehind side, const std::vector<LaneEndDistance<T>>& branches) {
  using std::abs;
  using std::min;
  ...
  LaneDirection ego_lane_direction = CalcLaneDirection<T>(
  ...
  for (int i = 0; i < traffic_poses.get_num_poses(); ++i) {
    ...
    const Lane* const traffic_lane =
        ego_lane->segment()
            ->junction()
            ->road_geometry()
            ->ToRoadPosition(MakeGeoPosition<T>(traffic_isometry), nullptr,
                             nullptr, nullptr)
            .lane;
    ...
    const LanePositionT<T> lane_position = traffic_lane->ToLanePositionT<T>(
    ...
    const LaneDirection traffic_ld =
        CalcLaneDirection<T>(traffic_lane, lane_position,
    ...
    for (auto branch_distance : branches) {
      ...
      const T distance_to_scan =
          min(scan_distance, abs(lane_sigma_v / T(kEgoSigmaVelocity)) *
...
const LaneEndSet* GetIncomingLaneEnds(const LaneDirection& lane_direction) {
