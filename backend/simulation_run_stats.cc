...
SimulationRunStats::SimulationRunStats(double start_simtime)
    : SimulationRunStats(start_simtime, Clock::now()) {}
...
void SimulationRunStats::StepExecuted(double simtime) {
  StepExecuted(simtime, Clock::now());
}
...
double SimulationRunStats::ElapsedSimtime() {
...
double SimulationRunStats::ElapsedRealtime() {
  if (executed_steps_ == 0) {
    return 0.0;
  }
