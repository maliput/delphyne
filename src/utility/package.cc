...
void BundledPackage::DoAdd(const ignition::common::URI& uri) {
  ...
  if (!ignition::common::exists(internal_dirpath)) {
    DELPHYNE_VALIDATE(ignition::common::createDirectories(internal_dirpath),
                      std::runtime_error,
                      "Cannot setup internal package structure");
  }
  DELPHYNE_VALIDATE(ignition::common::isDirectory(internal_dirpath),
                    std::runtime_error, "Cannot add below resource.");
  DELPHYNE_VALIDATE(ignition::common::copyFile(path, internal_path),
