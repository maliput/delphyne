...
struct AgentData2 {
  ...
  explicit AgentData2(const AgentType& t) : type(t) {}
...
template <typename T>
class TrajectoryAgent2 final : public systems::LeafSystem<T> {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(TrajectoryAgent2)
  ...
  /// system.  @default 0.01.
  TrajectoryAgent2(const AgentData2& agent_data,
                  const AgentTrajectory& trajectory,
                  double sampling_time_sec = 0.01);

  /// Scalar-converting copy constructor.  See @ref system_scalar_conversion.
  template <typename U>
  explicit TrajectoryAgent2(const TrajectoryAgent2<U>& other)
      : TrajectoryAgent2<T>(other.agent_data_, other.trajectory_) {}
 ...
 private:
  ...
  template <typename>
  friend class TrajectoryAgent2;

  const AgentData2 agent_data_;
