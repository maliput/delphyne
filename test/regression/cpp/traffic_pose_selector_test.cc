...
const drake::maliput::monolane::EndpointZ kEndZ{0., 0., 0., 0.};
...
static void SetDefaultDerivatives(PoseVector<AutoDiffXd>* ego_pose,
                                  PoseBundle<AutoDiffXd>* traffic_poses) {
...
TEST_F(TrafficPoseSelectorDragwayTest, TwoLaneDragway) {
  ...
  const double scan_ahead_distance = kDragwayLaneLength / 2.;
  {
    const std::map<AheadOrBehind, const ClosestPose<double>> closest_poses =
        TrafficPoseSelector<double>::FindClosestPair(
  ...
  const ClosestPose<double>& closest_pose =
      TrafficPoseSelector<double>::FindSingleClosestPose(
          get_lane(ego_pose, *road_), ego_pose, traffic_poses,
...
TEST_F(TrafficPoseSelectorDragwayTest, TwoLaneDragwayAutoDiff) {
  MakeDragway(2 /* num lanes */, kDragwayLaneLength);
  ...
  const AutoDiffXd scan_ahead_distance(kDragwayLaneLength / 2.);
  {
    const std::map<AheadOrBehind, const ClosestPose<AutoDiffXd>> closest_poses =
        TrafficPoseSelector<AutoDiffXd>::FindClosestPair(
            get_lane(ego_pose, *road_), ego_pose, traffic_poses,
            scan_ahead_distance, ScanStrategy::kPath);
    ...
    for (int i{0}; i < 3; ++i) {
      EXPECT_TRUE(test::CompareMatrices(expected_derivatives.col(i),
...
TEST_F(TrafficPoseSelectorDragwayTest, TestGetSigmaVelocity) {
  ...
  // Expect it to throw.
  EXPECT_THROW(
      TrafficPoseSelector<double>::GetSigmaVelocity({null_rp, velocity}),
      std::runtime_error);
  ...
  double sigma_v =
      TrafficPoseSelector<double>::GetSigmaVelocity({position, velocity});
  EXPECT_EQ(0., sigma_v);
...
MakeThreeSegmentMonolaneRoad(bool is_opposing) {
  Builder builder(
      RBounds(-std::abs(kEgoRPosition) - 2., std::abs(kEgoRPosition) + 2.),
      RBounds(-std::abs(kEgoRPosition) - 2., std::abs(kEgoRPosition) + 2.),
      HBounds(0., 5.) /* elevation bounds */, 0.01 /* linear tolerance */,
      0.01 /* angular_tolerance */);
  ...
  return builder.Build(drake::maliput::api::RoadGeometryId("ThreeLaneStretch"));
}
...
std::unique_ptr<const drake::maliput::api::RoadGeometry> BuildOnrampRoad() {
  ...
  const auto& lane6 =
      rb->Connect("lane6", kRoadOrigin,
                  ArcOffset(kArcRadius, -kArcLength / kArcRadius), kFlatZ);
  const auto& lane5 =
      rb->Connect("lane5", lane6->end(),
                  ArcOffset(kArcRadius, kArcLength / kArcRadius), kFlatZ);
  const auto& lane4 =
      rb->Connect("lane4", lane5->end(),
                  ArcOffset(kArcRadius, -kArcLength / kArcRadius), kFlatZ);
  const auto& lane3 =
      rb->Connect("lane3", lane4->end(),
                  ArcOffset(kArcRadius, kArcLength / kArcRadius), kFlatZ);
  const auto& lane2 =
      rb->Connect("lane2", lane3->end(),
                  ArcOffset(kArcRadius, -kArcLength / kArcRadius), kFlatZ);
  const auto& lane1 =
      rb->Connect("lane1", lane2->end(),
                  ArcOffset(kArcRadius, kArcLength / kArcRadius), kFlatZ);
  const double kLinearLength = 100.;
  const auto& lane0 = rb->Connect("lane0", lane1->end(), kLinearLength, kFlatZ);
  ...
  return rb->Build(drake::maliput::api::RoadGeometryId{"three_feeder_lanes"});
}

enum class LanePolarity { kWithS, kAgainstS };

// Appends a traffic car's pose to the provided traffic_poses.
void AddToTrafficPosesAt(int index, const Lane* traffic_lane,
                         double traffic_s_position, double traffic_speed,
  ...
  const Eigen::Vector3d translation_ahead(traffic_xyz.x(), traffic_xyz.y(),
                                          traffic_xyz.z());
...
void SetDefaultOnrampPoses(const Lane* ego_lane, const Lane* traffic_lane,
                           double traffic_speed, double ego_speed,
                           PoseVector<double>* ego_pose,
...
GTEST_TEST(TrafficPoseSelectorOnrampTest, CheckBranches) {
  ...
  for (const auto& it : test_cases) {
    ...
    for (const auto ego_polarity :
      ...
      SetDefaultOnrampPoses(GetLaneByLaneId(*road, it.ego_lane), traffic_lane,
                            10. /* traffic_speed */, 10. /* ego_speed */,
                            &ego_pose, &ego_velocity, &traffic_poses,
                            ego_polarity, it.traffic_orientation);
  ...
  for (const auto ego_polarity :
       {LanePolarity::kWithS, LanePolarity::kAgainstS}) {
    SetDefaultOnrampPoses(
        GetLaneByLaneId(*road, test_case.ego_lane), traffic_lane,
        10. /* traffic speed */, 10. /* ego_speed */, &ego_pose, &ego_velocity,
        &two_traffic_poses, ego_polarity, test_case.traffic_orientation);
    ...
    AddToTrafficPosesAt(1, other_traffic_lane,
                        ...
                        10. /* other traffic speed */, LanePolarity::kWithS,
                        &two_traffic_poses);
    ...
    CheckOnrampPosesInBranches(
        *road, ego_pose, two_traffic_poses, test_case.expected_traffic_lane,
